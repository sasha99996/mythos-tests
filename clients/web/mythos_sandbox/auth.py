...
class Auth(BasePage):
    ...
    LOGIN_BTN = (
        By.CSS_SELECTOR,
        "#auth-user",
        "Поле 'Логин' на форме Авторизация"
    )
    LOG_IN_BTN = (
        By.CSS_SELECTOR,
        "button[onclick*='openAuth()']",
    ...
    AUTH_WIDGET_LOGIN_FIELD = (By.CSS_SELECTOR, "#auth-user", "Поле 'Логин' на виджете авторизации")
    AUTH_WIDGET_PASSWORD_FIELD = (By.CSS_SELECTOR, "#auth-pass", "Поле 'Пароль' на виджете авторизации")
    ...
    def click_to_auth_widget_enter_btn(self, browser):
        ...
        auth_widget_enter_btn = self.find_element(browser, *self.LOG_IN_BTN, time_wait=20)
        auth_widget_enter_btn.click()

    def write_in_auth_widget_form_fields(self, browser, login=UserTuco.USER_NAME, password=UserTuco.PASSWORD):
        """Заполняет поля 'Логин' и 'Пароль' на виджете авторизации"""
        auth_widget_login_field = self.find_element(browser, *self.AUTH_WIDGET_LOGIN_FIELD)
        auth_widget_login_field.send_keys(login)

        auth_widget_password_field = self.find_element(browser, *self.AUTH_WIDGET_PASSWORD_FIELD)
    ...
    def click_to_auth_widget_submit_btn(self, browser):
        ...
        auth_widget_submit_btn = self.find_element(browser, *self.AUTH_WIDGET_LOG_IN_BTN)
