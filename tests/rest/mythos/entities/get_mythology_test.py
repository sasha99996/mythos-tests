...
from clients.rest.base_validation import (
    check_response_code,
    check_value_in_dicts,
    check_dicts_are_sorted_by_key,
)
import pytest
import allure
...
class TestMythology:
    ...
    def test_get_mythology_list_with_category_filter(self, category):
        with allure.step(
            "Шаг: получаем список героев и присваиваем значение переменной response"
        ):
        ...
        with allure.step(
            "Шаг: проверка что значение по ключу в каждом словаре совпадает с выбранным условием"
        ):
    ...
    def test_get_mythology_list_with_category_filter_1(self, sort, reverse):
        ...
        with allure.step("Шаг: проверка код ответа = 200"):
            check_response_code(response, 200)
        with allure.step(
            "Шаг: проверка что список словарей items отсортирован по указанному ключу key"
        ):
