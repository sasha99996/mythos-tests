...
from clients.rest.mythos_sandbox.entities import (
    partial_update_mythology_by_id,
    get_mythology_by_id,
)
...
def test_patch_mythology_with_auth(auth_user_tuco, mythology_id_by_user_tuco):
    response = partial_update_mythology_by_id(
        auth=auth_user_tuco, mythology_id=mythology_id_by_user_tuco, name="Зевс"
    )
...
def test_patch_mythology_without_auth(mythology_id_by_user_tuco):
    with allure.step(
        "Шаг: авторизация юзером tuco с помощью фикстуры и присваивание значения переменной response"
    ):
        response = partial_update_mythology_by_id(
            auth=None, mythology_id=mythology_id_by_user_tuco, name="Зевс"
        )
